  ...
  Future<SalesTableData> createSale({
    ...
    return _db.transaction(() async {
      ...
      final sale = SalesTableCompanion.insert(
        ...
        subTotal: Value(vatResult.subTotal),
        discountAmt: Value(vatResult.discountAmt),
        taxableAmount: Value(vatResult.taxableAmount),
        vatAmount: Value(vatResult.vatAmount),
  ...
  Future<SalesTableData> holdBill(BillDraft draft) async {
    ...
    return _db.transaction(() async {
      final sale = SalesTableCompanion.insert(
        ...
        taxableAmount: Value(vatResult.taxableAmount),
        vatAmount: Value(vatResult.vatAmount),
        grandTotal: Value(vatResult.grandTotal),
        paidAmount: const Value(0),
        changeAmount: const Value(0),
        netProfit: const Value(0),
        customerPan: Value(draft.customerPan),
        fiscalYear: draft.fiscalYear,
        fonepayQrRef: const Value(null),
